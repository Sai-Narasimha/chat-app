...
export const Login = () => {
    const { logoutUser, loginUser, loginError, isLoginLoading, updateLoginInfo, loginInfo } = useContext(AuthContext)
    return (
        <>
                ...
                <Row style={{ height: '90vh', justifyContent: 'center', paddingTop: '10%' }}>
                    <Col xs={6}>
                        <Stack gap={3}>
                            ...
                            {loginError?.error && <Alert variant='warning'><p>{loginError?.message}</p></Alert>}
                        </Stack>
                    </Col>
                </Row>
            </Form>
